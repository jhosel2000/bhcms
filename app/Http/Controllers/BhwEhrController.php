...
class BhwEhrController extends Controller
{
    ...
    {
        $bhwId = auth()->user()->bhw->id;

        // Get patients who have appointments with this BHW
        $query = Patient::whereHas('appointments', function ($query) use ($bhwId) {
            $query->where('bhw_id', $bhwId);
        })
        ->with([
            'ehrRecords' => function ($query) {
                $query->latest()->take(5); // Recent 5 records for summary
            },
            'appointments' => function ($query) use ($bhwId) {
                $query->where('bhw_id', $bhwId)
        ...
        return view('bhw.ehr.index', compact('patients'));
    }
    ...
    public function show(Request $request, Patient $patient)
    {
        ...
        // Ensure BHW has access to this patient
        ...
        return view('bhw.ehr.show', compact('patient', 'ehrRecords', 'recordTypeCounts', 'filters'));
    }
    ...
    public function create(Patient $patient)
    {
        ...
        return view('bhw.ehr.create', compact('patient', 'appointments'));
    }
    ...
    public function store(Request $request, Patient $patient)
    {
        ...
        $request->validate([
            'record_type' => ['required', Rule::in([
                'vital_signs', 'observation', 'referral', 'community_health',
                'urgent_care', 'other'
            ])],
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::error('Failed to send EHR update notification: ' . $e->getMessage());
        }

        return redirect()->route('bhw.ehr.show', $patient)->with('success', 'EHR record created successfully.');
    }
    ...
    public function edit(Patient $patient, EhrRecord $ehrRecord)
    {
        ...
        // Only allow editing of BHW-created records
        if ($ehrRecord->created_by_role !== 'bhw') {
        ...
        return view('bhw.ehr.edit', compact('patient', 'ehrRecord'));
    }
    ...
    public function update(Request $request, Patient $patient, EhrRecord $ehrRecord)
    {
        ...
        return redirect()->route('bhw.ehr.show', $patient)->with('success', 'EHR record updated successfully.');
    }
    ...
    public function destroy(Patient $patient, EhrRecord $ehrRecord)
    {
        ...
        // Only allow deletion of BHW-created records
        if ($ehrRecord->created_by_role !== 'bhw') {
        ...
        return redirect()->route('bhw.ehr.show', $patient)->with('success', 'EHR record deleted successfully.');
    }
}
