/**
 * Appointment Availability Service - Fixed Version
...
class AppointmentAvailabilityService {
    ...
    async checkSlotAvailability(providerId, providerType, date, time, durationMinutes = 30) {
        try {
            ...
            const response = await fetch(`${this.baseUrl}/check-slot?${params}`, {
                method: 'GET',
                headers: {
    ...
    /**
     * Get available time slots using the generic endpoint
     */
    async getSlotsByType(providerType, providerId, date) {
        try {
            const response = await fetch(`${this.baseUrl}/slots/${providerType}/${providerId}?date=${date}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': this.csrfToken,
            ...
            console.error('Error getting slots by type:', error);
            return {
                success: false,
                data: { available_slots: [] }
            };
