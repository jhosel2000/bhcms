                                    ...
                                    @foreach($records as $record)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                @can('view', $record)
                                                    <a href="{{ route('patient.maternal.show', $record) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                                                @endcan
                                            </td>
                                        </tr>
                                    @endforeach
                                ...
                                <div class="bg-gray-50 rounded-lg p-4 border">
                                    <div class="flex justify-between items-start mb-2">
                                        <div class="flex space-x-2">
                                            @can('view', $record)
                                                <a href="{{ route('patient.maternal.show', $record) }}" class="text-indigo-600 hover:text-indigo-900 text-sm">View</a>
                                            @endcan
                                        </div>
                                    </div>
