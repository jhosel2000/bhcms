        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('bhw.appointments.update', $appointment) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                                ...
                                <div>
                                    ...
                                    <select name="midwife_id" id="midwife_id" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                        ...
                                        @foreach($midwives as $midwife)
                                            <option value="{{ $midwife->id }}" {{ $appointment->midwife_id == $midwife->id ? 'selected' : '' }}>{{ $midwife->user->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="appointment_date" id="appointment_date" value="{{ $appointment->appointment_date->format('Y-m-d') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="time" name="appointment_time" id="appointment_time" value="{{ $appointment->appointment_time->format('H:i') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="reason" id="reason" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>{{ $appointment->reason }}</textarea>
                            @error('reason')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="urgency_level" id="urgency_level" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="normal" {{ $appointment->urgency_level == 'normal' ? 'selected' : '' }}>Normal</option>
                                <option value="urgent" {{ $appointment->urgency_level == 'urgent' ? 'selected' : '' }}>🚨 Urgent</option>
                                <option value="maternal" {{ $appointment->urgency_level == 'maternal' ? 'selected' : '' }}>🤰 Maternal Care</option>
                            </select>
                            <p class="mt-1 text-sm text-gray-600">Select urgency level for this appointment.</p>
                        </div>

                        <div class="mb-4">
                            <label for="uploaded_files" class="block text-sm font-medium text-gray-700">Additional Supporting Files (optional)</label>
                            ...
                            <p class="mt-1 text-sm text-gray-600">Upload additional lab results, referral letters, or other supporting documents (max 5MB each).</p>
                            @if($appointment->uploaded_files && count($appointment->uploaded_files) > 0)
                                <p class="mt-1 text-sm text-gray-600">Existing files: {{ count($appointment->uploaded_files) }} file(s) uploaded.</p>
                            @endif
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="notes" id="notes" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ $appointment->notes }}</textarea>
                        </div>
